...
veryOldInventors = inventors.filter((oldInv) => {
    return oldInv.year > 1500 && oldInv.year < 1599;
})
...
firstLast = people.map((name) => {
    let nameSplit = (name.split(',')).reverse();
...
totalYearsLived = inventors.reduce(function(acc, year) {
    let lifeSpan = year.passed - year.year;
    return acc + lifeSpan;
}, 0);
